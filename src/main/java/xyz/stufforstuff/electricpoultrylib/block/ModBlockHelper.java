...
public class ModBlockHelper {
    ...
    public static Block createExperienceDroppingBlock(String blockId, BlockBehaviour.Properties blockProperties, UniformInt uniformInt, float strength, SoundType soundtype, boolean shouldRegisterItem) {
        ResourceKey<Block> blockKey = keyOfBlock(blockId);

        Block block = new DropExperienceBlock(uniformInt, blockProperties.strength(strength).sound(soundtype).setId(blockKey));

        if (shouldRegisterItem) {
            ModItemHelper.createBlockItem(blockId, block, new Item.Properties().useBlockDescriptionPrefix());
        }
